...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[sp_CRMLoad_SaveResults_HouseHold]
as
--Fix Problems with SFID mapping with Contact GUID MATCH
UPDATE a
SET [a].[crm_id] = [a].[SSB_CRMSYSTEM_HOUSEHOLD_ID]
--SELECT *
FROM dbo.HOUSEHOLD a 
INNER JOIN dbo.[HOUSEHOLD_CRMResults] b ON 
--[a].[salesforce_ID] = b.[Id] 
[a].[SSB_CRMSYSTEM_HOUSEHOLD_ID] = b.[SSB_CRMSYSTEM_HOUSEHOLD_ID__c]
...
WHERE [a].[SSB_CRMSYSTEM_HOUSEHOLD_ID] <> b.[SSB_CRMSYSTEM_HOUSEHOLD_ID__c]
AND LoadType = 'Upsert'
...
--SELECT b.*
FROM dbo.HOUSEHOLD a
INNER JOIN dbo.[HOUSEHOLD_CRMResults] b ON a.[SSB_CRMSYSTEM_HOUSEHOLD_ID] = b.[SSB_CRMSYSTEM_HOUSEHOLD_ID__c]
