...
FROM dbo.Contact c
INNER JOIN prodcopy.contact pcc WITH (NOLOCK)
ON c.crm_id = pcc.Id
...
--SELECT COUNT(*)
FROM dbo.contact a
LEFT JOIN prodcopy.vw_contact b WITH (NOLOCK)
ON a.[crm_id] = b.id
where b.id IS NULL
...
--SELECT COUNT(*)
FROM dbo.contact a
INNER JOIN (select cc.*, ROW_NUMBER() OVER(PARTITION BY SSB_CRMSYSTEM_Contact_ID__C ORDER BY CASE WHEN d.FirstName = 'ETL' THEN 0 WHEN d.IsActive = 0 THEN 1 ELSE 99 END DESC, cc.createddate) xRank FROM prodcopy.vw_contact cc WITH (NOLOCK)
			JOIN [ProdCopy].[vw_User] d WITH (NOLOCK)-- order by 5
			ON cc.OwnerId = d.Id
...
--FROM dbo.contact c
--LEFT JOIN (SELECT ssb_crmsystem_contact_id FROM dbo.vwDimCustomer_ModAcctId  WITH (NOLOCK) WHERE SourceSystem = 'Monumental PC_SFDC Account' AND IsDeleted = 0) ssb
...
--SELECT COUNT(*)
FROM dbo.contact a
INNER JOIN dbo.vwdimcustomer_modacctid b WITH (NOLOCK) ON a.SSB_CRMSYSTEM_Contact_ID = b.SSB_CRMSYSTEM_Contact_ID
LEFT JOIN (SELECT crm_id FROM dbo.contact WHERE crm_id <> SSB_CRMSYSTEM_Contact_ID) c ON b.ssid = c.[crm_id]
WHERE b.SourceSystem = 'Monumental PC_SFDC Contact' AND ISNULL(a.crm_id, '') != b.ssid --updateme
AND c.[crm_id] IS NULL 
...
from dbo.contact c
INNER JOIN dbo.vwDimCustomer_ModAcctId dc WITH (NOLOCK) ON c.SSB_CRMSYSTEM_Contact_ID = dc.SSB_CRMSYSTEM_CONTACT_ID
WHERE dc.SourceSystem = 'Monumental PC_SFDC Contact' --updateme
AND c.crm_id = dc.SSB_CRMSYSTEM_CONTACT_ID
