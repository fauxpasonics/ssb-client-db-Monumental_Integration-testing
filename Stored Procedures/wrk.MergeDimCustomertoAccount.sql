...
, SSID_Winner, SourceSystem, SSB_CRMSYSTEM_CONTACT_ID, SSB_CRMSYSTEM_HOUSEHOLD_ID)
...
, SSB_CRMSYSTEM_CONTACT_ID
, SSB_CRMSYSTEM_HOUSEHOLD_ID
, ROW_NUMBER() OVER(PARTITION BY a.SSB_CRMSYSTEM_CONTACT_ID ORDER BY a.SSB_CRMSYSTEM_CONTACT_ID, a.SSB_CRMSYSTEM_PRIMARY_FLAG, dimcustomerid) rownum
-- SELECT COUNT(*)
INTO #tmpTest
...
where 1=1
and a.SSB_CRMSYSTEM_ACCT_PRIMARY_FLAG = 1
--AND (a.IsBusiness = 1 or ISNULL(a.CompanyName,'') <> '') --Only where we only want to push Business Accounts
...
AND a.SSB_CRMSYSTEM_CONTACT_ID NOT IN (SELECT DISTINCT SSB_CRMSYSTEM_CONTACT_ID FROM wrk.customerWorkingList)


INSERT INTO wrk.customerWorkingList (DimCustomerID, SSB_CRMSYSTEM_ACCT_ID, IsPersonAccount, IsBusinessAccount, MDM_UpdatedDate, CRMProcess_UpdatedDate
, SSID_Winner, SourceSystem, SSB_CRMSYSTEM_CONTACT_ID, SSB_CRMSYSTEM_HOUSEHOLD_ID)
SELECT DimCustomerID,	SSB_CRMSYSTEM_ACCT_ID,	IsPersonAccount,	IsBusinessAccount,	MDM_LastUpdated,	SFDCProcess_UpdatedDate,	SSID_Winner,	SourceSystem,	SSB_CRMSYSTEM_CONTACT_ID,	SSB_CRMSYSTEM_HOUSEHOLD_ID
FROM #tmpTest WHERE rownum = 1
...
, case WHEN ISNULL(dc.CompanyName,'') != '' THEN CompanyName ELSE ISNULL(dc.FirstName,'') + ' ' + ISNULL(dc.LastName,'') + ' ' + ISNULL(dc.Suffix,'') END AS  FullName
, FirstName
, LastName
, Suffix
...
, target.SSB_CRMSYSTEM_HOUSEHOLD_ID = source.SSB_CRMSYSTEM_HOUSEHOLD_ID
