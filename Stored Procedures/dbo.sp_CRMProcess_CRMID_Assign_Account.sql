...
--SELECT COUNT(*)
FROM dbo.account a
LEFT JOIN prodcopy.Fan_Profile__c b
ON a.[crm_id] = b.id
WHERE b.id IS NULL
...
--SELECT COUNT(*)
FROM dbo.account a
INNER JOIN prodcopy.Fan_Profile__c b ON a.SSB_CRMSYSTEM_ACCT_ID = b.SSB_CRMSYSTEM_ACCT_ID__C
...
WHERE b.SourceSystem = 'Monumental PC_SFDC FanProfile' AND ISNULL(a.crm_id, '') != b.ssid
AND c.[crm_id] IS NULL 
AND b.isdeleted = 0
