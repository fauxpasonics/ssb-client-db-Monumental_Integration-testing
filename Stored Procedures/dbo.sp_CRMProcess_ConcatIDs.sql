...
SELECT        CASE WHEN @ObjectType = 'Account' THEN SSB_CRMSYSTEM_ACCT_ID WHEN @ObjectType = 'Household' THEN ssb_crmsystem_household_id ELSE [SSB_CRMSYSTEM_CONTACT_ID] END GUID, CAST(DimCustomerId AS VARCHAR(100)) AS DimCustomerID, CAST(SSID AS VARCHAR(50)) AS SSID
, a.SourceSystem
INTO #tmpDimCustIDs
...
SELECT GUID, a.DimCustomerID, a.[SourceSystem], CAST(a.SSID as varchar(50)) AS SSID, b.AccountId CustomID1, CASE WHEN @ObjectType = 'Household' THEN b.SSB_CRMSYSTEM_HOUSEHOLD_PRIMARY_FLAG else SSB_CRMSYSTEM_ACCT_PRIMARY_FLAG END PrimaryFlag
...
WHERE TM.[GUID] = z.[GUID] AND tm.[SourceSystem] = 'TM' --updateme
...
FROM (SELECT CustomID1,[GUID], MAX(CAST(Primary_Flag as int)) Primary_Flag 
		FROM stg.tbl_CRMProcess_NonWinners WHERE [SourceSystem] = 'TM' GROUP by CustomID1,[GUID]) acct --updateme
...
--INTO #tmpA
FROM (SELECT DISTINCT GUID FROM [stg].tbl_CRMProcess_NonWinners WHERE GUID IS NOT null
...
INNER JOIN stg.[tbl_CRMProcess_ConcatIDs] b ON a.[SSB_CRMSYSTEM_HOUSEHOLD_ID] = b.GUID
